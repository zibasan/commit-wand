...
if (majorVersion < 22) {
  console.error(
    chalk.bgRed.black(`${error} FATAL `) + chalk.red(`This CLI requires Node.js v22 or higher.`)
  );
  console.error(chalk.yellow(`You are currently running Node.js v${currentNodeVersion}.`));
  console.error(
    chalk.yellow(`Please upgrade your Node.js version (v22 or higher) to use this CLI.`)
  );
  process.exit(1);
}
...
if (majorVersion > 22) {
  console.warn(
    warn +
      chalk.yellow(
        ` You are running Node.js v${chalk.red(currentNodeVersion)}.\n` +
